...
df_sc <- read_csv("C:/Users/User/Desktop/seungchon_2016-2023.csv", 
                  show_col_types = FALSE, locale = locale(encoding = "EUC-KR"))
df_js <- read_csv("C:/Users/User/Desktop/juksan_2016-2023.csv", 
                  show_col_types = FALSE, locale = locale(encoding = "EUC-KR"))
...
preprocess_data <- function(df, site_name_eng) {
  df %>%
    ...
    filter(Year >= 2016 & Year <= 2022) # 2016~2022년 데이터만 추출 (Training Period)
}
...
p1 <- ggplot(df_total, aes(x = factor(Month), y = Chla, fill = Site)) +
  ...
  labs(
    title = "(a) Monthly Distribution of Chlorophyll-a (2016-2022)",
    x = "Month",
