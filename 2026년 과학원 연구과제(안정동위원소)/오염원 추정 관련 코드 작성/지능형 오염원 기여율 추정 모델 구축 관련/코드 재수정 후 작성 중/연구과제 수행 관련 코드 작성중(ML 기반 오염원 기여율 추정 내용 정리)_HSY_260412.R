...
for (m in all_methods) {
  # 표준화된 시료(single_matrix_scaled)를 입력으로 사용
...
if (file.exists("C:/Users/User/Desktop/sample data 1_HSY.csv")) {
  ...
  cat("모든 모델에 대한 배치 분석 및 CSV 저장이 완료되었습니다.\n")
} else {
  warning("작업 디렉토리에 'unknown_data.csv'가 없어 4-2 단계를 건너뜁니다.")
}
...
if (file.exists(target_file)) {
  ...
  # 학습 데이터의 기준을 적용하여 패턴 분석용 시료 표준화
  ...
  cat("고정/임의효과 기반 요약 분석 및 그룹별 통계 파일 저장이 완료되었습니다.\n")
} else {
