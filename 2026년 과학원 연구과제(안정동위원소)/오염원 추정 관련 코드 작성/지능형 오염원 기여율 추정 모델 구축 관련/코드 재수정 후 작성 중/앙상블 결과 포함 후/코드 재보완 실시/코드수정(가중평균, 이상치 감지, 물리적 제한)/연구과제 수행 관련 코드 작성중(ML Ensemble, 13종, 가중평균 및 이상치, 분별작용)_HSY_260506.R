...
generate_virtual_data_advanced <- function(n_samples = 5000) {
  ...
  # [추가] TDF 설정 (예시 데이터: d15N, d18O 순서)
  # 연구 지역의 특성에 맞춰 평균(tdf_mean)과 표준편차(tdf_sd)를 조정하세요.
  tdf_mean <- matrix(c(
    0.0, 0.0, # Manure TDF
    0.0, 0.0, # Sewage TDF
    0.0, 0.0, # Fertilizer TDF
    0.0, 0.0, # Soil TDF
    0.0, 0.0  # Rain TDF
  ), nrow=5, byrow=TRUE)
  ...
  # 3. 혼합 법칙 적용 및 데이터 생성 (TDF 반영)
  # 수식: Mean = Σ p * (Source_mean + TDF_mean)
  M_mean <- props %*% (src_mean + tdf_mean)
  
  # 수식: Var = Σ p^2 * (Source_sd^2 + TDF_sd^2)
  # 오염원 오차와 분별작용 오차의 독립성을 가정하여 합산합니다.
  M_var <- (props^2) %*% (src_sd^2 + tdf_sd^2)
...
# =====================================================================
# [Step 3-1] K-fold-crossvalidation 기반 통합 성능 평가 및 가중치 산출
# =====================================================================
cat("\n=== [Step 3-1. 5-Fold Cross-Validation 통합 성능 평가 및 가중치 산출] ===\n")
set.seed(2026)
cv_folds <- createFolds(1:nrow(dataset_adv), k = 5, list = TRUE)

# 각 모델의 1/RMSE 값을 저장할 벡터 생성
model_weights <- c()
...
for (m in all_methods) {
  ...
  # 평균 1/RMSE 값을 해당 모델의 가중치로 저장
  model_weights[m] <- 1 / (avg_m["RMSE"] + 1e-10) # 0으로 나누기 방지
  
  cat(sprintf("[CV] %-10s | MAE: %.4f | RMSE: %.4f | MAPE: %.2f%% | IOA: %.4f\n", 
              m, avg_m["MAE"], avg_m["RMSE"], avg_m["MAPE"], avg_m["IOA"]))
}

# 1/RMSE 값을 기반으로 가중치 정규화 (총합을 1로 만듦)
model_weights <- model_weights / sum(model_weights) 

cat("\n[모델별 산출된 최종 가중치 (1/RMSE 기준)]\n")
print(round(model_weights, 4))
...
# ---------------------------------------------------------------------
cat("\n=== [4-1. 단일 임의 시료 기반 알고리즘별 & 가중 앙상블 추정 결과] ===\n")
...
# 가중 평균(Weighted Mean) 적용
single_ens_weighted_mean <- Reduce("+", lapply(all_methods, function(m) {
  single_preds_list[[m]] * model_weights[m]
}))

# 모델 간 변동성 확인용 (가중 표준편차)
single_preds_array <- simplify2array(single_preds_list)
single_ens_sd <- apply(single_preds_array, c(1, 2), function(x) {
  w_mean <- sum(x * model_weights)
  w_var <- sum(model_weights * (x - w_mean)^2) 
  return(sqrt(w_var))
})

cat("\n[Weighted Ensemble Mean (IOA 가중 평균)]: ")
print(round(single_ens_weighted_mean, 4))
cat("[Ensemble SD (전체 모델 표준편차)]: ")
...
if (file.exists("C:/Users/User/Desktop/sample data 1_HSY.csv")) {
  ...
  # 가중 평균(Weighted Mean) 적용
  batch_ens_weighted_mean <- Reduce("+", lapply(all_methods, function(m) {
    batch_preds_list[[m]] * model_weights[m]
  }))
  
  # 1. 리스트를 3차원 배열(Array)로 변환 (기존과 동일)
  batch_preds_array <- simplify2array(batch_preds_list)
  # 2. 가중 표준편차(Weighted SD)를 계산하는 함수 적용
  batch_ens_sd <- apply(batch_preds_array, c(1, 2), function(x) {
    # x는 특정 시료, 특정 오염원에 대한 13개 모델의 예측값 (길이 13인 벡터)
    # a. 가중 평균 계산
    w_mean <- sum(x * model_weights)
    # b. 가중 분산 계산: 가중치 * (예측값 - 가중평균)^2 의 합
    w_var <- sum(model_weights * (x - w_mean)^2) 
    # c. 가중 표준편차 반환 (분산에 루트 적용)
    return(sqrt(w_var))
  })
  
  colnames(batch_ens_weighted_mean) <- paste0("EnsWMean_", colnames(dataset_adv_Y))
  colnames(batch_ens_sd) <- paste0("EnsWSD_", colnames(dataset_adv_Y))
  
  final_ensemble_output <- cbind(unknown_samples_df, 
                                 round(batch_ens_weighted_mean, 4), 
                                 round(batch_ens_sd, 4))
  
  write.csv(final_ensemble_output, "Final_Contribution_Weighted_Ensemble.csv", row.names = FALSE)
  cat("개별 모델(13종) 및 가중 평균 통합 앙상블 배치 분석 결과 저장이 완료되었습니다.\n")
...
if (file.exists(target_file)) {
  ...
  # 가중 평균(Weighted Mean) 적용
  sample_ens_weighted_mean <- Reduce("+", lapply(all_methods, function(m) {
    sample_preds_list[[m]] * model_weights[m]
  }))
  colnames(sample_ens_weighted_mean) <- colnames(dataset_adv_Y) 
  
  ens_effect_output <- cbind(sample_df, round(sample_ens_weighted_mean, 4))
  ...
  for (effect in target_effects) {
    if (effect %in% colnames(ens_effect_output)) {
      ens_summary_df <- ens_effect_output %>%
        group_by(.data[[effect]]) %>%
        summarise(across(starts_with("p_"), 
                         list(W_Mean = ~mean(.x, na.rm = TRUE), # 가중 평균값의 통계임을 표기
                              W_SD = ~sd(.x, na.rm = TRUE)),
                         .names = "{.col}_{.fn}")) %>%
        mutate(across(where(is.numeric), ~round(.x, 4)))
      
      summary_filename <- paste0("Summary_Contribution_Weighted_Ensemble_by_", effect, ".csv")
      write.csv(ens_summary_df, summary_filename, row.names = FALSE)
    }
  }
  
  cat("개별 모델 및 가중 앙상블의 그룹별 통계 파일 저장이 완료되었습니다.\n")
} else {
  warning(sprintf("작업 디렉토리에 '%s'가 없어 4-3 단계를 건너뜁니다.", target_file))
}

# ---------------------------------------------------------------------
# [Step 4-4] 이상치 감지(OOD Detection) 및 신뢰도 점수(Confidence Score) 산출
# ---------------------------------------------------------------------
cat("\n=== [4-4. 이상치 감지(OOD) 및 앙상블 신뢰도 점수 산출] ===\n")

# 4-2 단계에서 생성된 데이터(unknown_matrix_scaled, batch_ens_sd 등)가 존재하는지 확인
if (exists("unknown_matrix_scaled") && exists("batch_ens_sd") && exists("unknown_samples_df")) {
  
  # -----------------------------------------------------------
  # 1. Mahalanobis Distance 기반 OOD (이상치) 판별
  # -----------------------------------------------------------
  # 학습 데이터(dataset_adv_X)의 평균과 공분산 행렬 계산
  train_center <- colMeans(dataset_adv_X)
  train_cov <- cov(dataset_adv_X)
  
  # 미지 시료의 마할라노비스 거리(MD) 산출
  md_dist <- mahalanobis(unknown_matrix_scaled, center = train_center, cov = train_cov)
  
  # 카이제곱 분포를 이용한 p-value 산출 (자유도 df = 변수 개수 = 2)
  # p-value가 0.05 미만이면 학습 데이터의 95% 신뢰구간을 벗어난 것으로 간주
  p_values <- pchisq(md_dist, df = ncol(dataset_adv_X), lower.tail = FALSE)
  
  # OOD 상태 라벨링
  ood_status <- ifelse(p_values < 0.05, "Outlier (OOD)", "Normal (In-Dist)")
  
  # -----------------------------------------------------------
  # 2. 모델 합의도 기반 신뢰도 점수(Confidence Score) 산출
  # -----------------------------------------------------------
  # 13개 앙상블 모델 간의 예측 표준편차(SD) 평균 산출
  # 각 시료당 5개 오염원에 대한 SD의 평균값을 모델 불확실성 지표로 사용
  mean_batch_sd <- rowMeans(batch_ens_sd)
  
  # 신뢰도 점수 변환 (0 ~ 100점 척도)
  # 이론상 SD가 0이면 100점. SD가 0.4(40% 편차) 이상이면 0점에 가깝도록 스케일링
  max_expected_sd <- 0.4 
  confidence_score <- pmax(0, pmin(100, (1 - (mean_batch_sd / max_expected_sd)) * 100))
  
  # OOD인 경우 신뢰도 점수에 페널티(절반 감점)를 주어 경각심 부여 (선택적)
  confidence_score <- ifelse(ood_status == "Outlier (OOD)", confidence_score * 0.5, confidence_score)
  
  # -----------------------------------------------------------
  # 3. 결과 테이블 병합 및 저장
  # -----------------------------------------------------------
  ood_results_df <- data.frame(
    Mahalanobis_Dist = round(md_dist, 4),
    P_Value = round(p_values, 4),
    OOD_Status = ood_status,
    Ensemble_Mean_SD = round(mean_batch_sd, 4),
    Confidence_Score = round(confidence_score, 1)
  )
  
  # 4-2에서 생성한 앙상블 최종 결과 파일에 OOD 및 신뢰도 컬럼 추가
  # (기존 데이터프레임과 행 순서가 동일하므로 cbind 사용)
  final_ood_output <- cbind(unknown_samples_df, ood_results_df)
  
  # 4-2의 예측 결과를 병합하여 완전히 통합된 파일로 만들려면 아래 코드 사용
  if (exists("batch_ens_weighted_mean")) {
    final_integrated_output <- cbind(unknown_samples_df, 
                                     round(batch_ens_weighted_mean, 4),
                                     round(batch_ens_sd, 4),
                                     ood_results_df)
    write.csv(final_integrated_output, "Final_Contribution_Ensemble_with_OOD.csv", row.names = FALSE)
  } else {
    write.csv(final_ood_output, "Final_OOD_Confidence_Only.csv", row.names = FALSE)
  }
  
  cat("미지 시료에 대한 OOD 감지 및 신뢰도 점수 산출이 완료되었습니다.\n")
  cat("결과 파일: Final_Contribution_Ensemble_with_OOD.csv (저장 완료)\n")
  
} else {
  warning("4-2 단계의 미지 시료 분석 결과가 메모리에 없어 4-4 단계를 건너뜁니다.")
}
