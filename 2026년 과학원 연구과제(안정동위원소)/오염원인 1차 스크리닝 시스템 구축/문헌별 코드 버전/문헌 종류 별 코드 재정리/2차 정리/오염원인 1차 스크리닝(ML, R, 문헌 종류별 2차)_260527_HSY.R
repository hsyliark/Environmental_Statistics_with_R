...
# ------------------------------------------------------------------------------
# [Core Logic] 문헌 기반의 다중 지표 표준 판별 규칙 함수 (Ref 22, 23 고도화 업데이트 반영)
assign_source_weighted <- function(row_list, lit_case = 3) {
  ...
  p_score <- c(P1 = 1000, P2 = 800, P3 = 600, P4 = 400, P5 = 200)
  
  # 점수 누적용 리스트 초기화 및 동적 할당 함수
  scores <- list()
  add_score <- function(name, score) {
    if (score <= 0) return() # 가중치가 0인 문헌 조건(예: Case 1에서 국외 문헌)은 무시
  ...
  # ---------------------------------------------------------
  # [Ref 1, 6(Int), 16(Dom)] 대기 강하물 및 초기 강우
  if (safe_val(o18) > 15.0) add_score("대기유입/초기강우", p_score["P1"] * (w_int + w_dom))
  
  # [Ref 22(Dom)] 대기강하물 (국립환경과학원 2024년 1차년도 로컬 동위원소 기준)
  # 영산강/섬진강 수계 특이적 대기강하물 End-member 범위 반영 
  if (safe_val(n15) >= -10.0 && safe_val(n15) <= 5.0 && safe_val(o18) >= 50.0) {
    add_score("대기유입/초기강우", p_score["P1"] * (w_dom * 1.0))
  }
  
  # [Ref 2, 3(Int), 21(Dom)] 미생물 탈질작용 (산소/질소 동위원소 동반 상승)
  ...
  if (safe_val(n15) >= 8.0 && safe_val(n15) <= 22.0 && safe_val(o18) >= 0.0 && safe_val(o18) <= 15.0) {
    # 분뇨와 하수가 혼재되는 구간이므로 양쪽에 점수를 균등 배분하여 앙상블 판별력 상승
  ...
  # [Ref 22(Dom)] 토양유기물 (국립환경과학원 2024년 1차년도 로컬 동위원소 기준)
  # 토양/식생 기원의 특성을 가지는 구간 정립 적용
  if (safe_val(n15) >= 1.0 && safe_val(n15) <= 8.0 && safe_val(o18) >= 0.0 && safe_val(o18) <= 12.0) {
    add_score("토양유기물/야생동물배설물", p_score["P4"] * (w_dom * 1.0))
  }
  
  # [Ref 23(Dom)] 토지계 비점오염 (국립환경과학원 2025년 2차년도 수질 연계 복합 판별 기준)
  # 비점오염원(토지계) 기여율 분석 결과를 반영하여 동위원소와 TOC, TP의 상관성 기반 판별 룰 적용
  if (safe_val(n15) >= 2.0 && safe_val(n15) <= 7.0 && (safe_val(tp) < 0.05 || safe_val(toc) >= 3.0)) {
    add_score("토지계 비점오염(강우유출)", p_score["P4"] * (w_dom * 1.0))
  }
  
  # ---------------------------------------------------------
  # [Priority 5] Fallback (동위원소 미약/결측 시 보완 및 복합 오염 판정)
  # ---------------------------------------------------------
  # [Ref 7(Int)] 다중 오염원 혼합 추정 (국외 문헌 단독 지표)
  if (!is.na(n15) && n15 > 7.0) add_score("복합오염(하수/분뇨 우세)", p_score["P5"] * w_int)
  if (!is.na(n15) && n15 <= 7.0) add_score("복합오염(비점오염 우세)", p_score["P5"] * w_int)
  
  # 동위원소 NA인 상태에서도 주요 수질 항목 지시자가 강할 경우 하드캐리 판정 (국내외 공통 적용)
  if (safe_val(cl) > 50 || safe_val(tp) > 0.1) add_score("복합오염(수화학적 점오염 징후)", p_score["P5"] * (w_int*0.5 + w_dom*0.5))
  if (safe_val(no3) > 2.0) add_score("복합오염(수화학적 비점 징후)", p_score["P5"] * (w_int*0.5 + w_dom*0.5))
...
# 연구팀 요청사항: 3가지 Case 결과물 동시 생성 (후속 비교 분석 및 논문화 작업용)
