...
apply_kendall_mask <- function(data) {
  data %>% mutate(
    Kendall_Pass = case_when(
      구분 == "강우계" & (d15N >= -15 & d15N <= 15) & (d18O >= 25 & d18O <= 100) ~ TRUE,
      구분 == "토양계" & (d15N >= 0 & d15N <= 6)  & (d18O >= -10 & d18O <= 15) ~ TRUE,
      구분 == "축산계" & (d15N >= 5 & d15N <= 25)   & (d18O >= -10 & d18O <= 15) ~ TRUE,
      구분 == "생활계" & (d15N >= 5 & d15N <= 25)   & (d18O >= -10 & d18O <= 15) ~ TRUE,
      구분 == "농업계" & (d15N >= -6 & d15N <= 6)  & (d18O >= -10 & d18O <= 25) ~ TRUE,
...
df_step2 <- df_step1 %>%
  ...
  group_modify(~ {
    sub_df <- .x
    if (nrow(sub_df) >= 5) {
      # 1) 데이터 프레임을 C/C++ 연산용 수치형 행렬(Matrix)로 변환 (list 에러 방지)
      mat_coords <- as.matrix(sub_df[, c("d15N", "d18O")])
      
      # 2) 표준 커널밀도 추정 수행 (eval.points 미사용 -> fhat$cont 정상 산출)
      fhat <- ks::kde(x = mat_coords)
      
      # 3) 수치형 행렬을 predict()에 전달하여 각 시료 위치의 확률밀도 산출
      eval_pts <- predict(fhat, x = mat_coords)
      
      # 4) 상위 95% 밀도 영역(하위 5% 컷오프 이상) 포함 여부 판단 (길이 13 벡터 정상 생성)
      sub_df$KDE_Pass <- eval_pts >= fhat$cont["5%"]
    } else {
      sub_df$KDE_Pass <- TRUE
    }
    return(sub_df)
  }) %>% 
  ungroup()
...
write.csv(df_final, "C:/Users/User/Desktop/오염원 데이터 선별/오염원_선별결과_GateReview_Log.csv", 
...
write.csv(summary_table, "C:/Users/User/Desktop/오염원 데이터 선별/오염원_선별_단계별_집계.csv", 
          row.names = FALSE, fileEncoding = "CP949")
...
ggsave("C:/Users/User/Desktop/오염원 데이터 선별/오염원_선별결과_KendallPlot.png", 
